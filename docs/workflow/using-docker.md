...
Чтобы выполнить кросс-сборку с использованием Docker, убедитесь, что вы выбрали необходимый образ для системы сборки. Необходимые команды представлены [в соответствующей главе](/docs/workflow/building/coreclr/cross-building.md). При работе с Docker вам не нужно генерировать *ROOTFS*, так как образы для кросс-сборки включают его по умолчанию.